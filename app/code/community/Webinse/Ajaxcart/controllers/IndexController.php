...
/**
 * Webinse_Webinse_Ajaxcart_controllers_IndexController
 * PHP Version 5.5.9
 *
 * @category  Webinse
...
class Webinse_Ajaxcart_IndexController extends Mage_Checkout_CartController
{
    ...
    public function addAction()
    {
        ...
        $cart = $this->_getCart();
        $productCart = Mage::getModel('checkout/cart')->getQuote();
        foreach ($productCart->getAllItems() as $item) {
            $productName = $item->getProduct()->getName();
            $qtyStock = $item->getProduct()->getStockItem()->getQty();
            $qtyCart = $item->getQty();
            if ($qtyCart > $qtyStock) {
                $response['status'] = 'ERROR';
                $response['message'] = $this->__('Check your cart. The requested quantity for ' . $productName . ' is not available in the your cart.');
            ...
            if ($this->getRequest()->getParam('id') > 0) {
                ...
                if ($product->getId()) {
                    ...
                    $response['message'] = $this->__(
                        'The product %s has been removed from comparison list.',
